...
# открываем документ
doc = bs(codecs.open('Сериал Рик и Морти 1-4 сезон 1-10 серия смотреть онлайн в хорошем качестве.html', encoding='utf-8', mode='r').read(), 'html.parser')

print('Количество положительных отзывов:', doc.select('.psc')[0].decode_contents().strip())

print('Количество отрицательных отзывов:', doc.select('.msc')[0].decode_contents().strip())
...
for node in doc.select('.comments-tree-item'):
    ...
    print("пользователь '", author, "' написал:", message, "рэйтинг", rating, "\n\n")
